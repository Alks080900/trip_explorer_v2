...
const TripDetails = ({ isLoggedIn }) => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      if (userId && token && isLoggedIn) {
        const response = await fetch(`${API_URL}/api/v1/notes/user/${userId}/trip/${tripId}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const toggleFavorite = async () => {
    ...
    if (isFavorite && favoriteId) {
      const response = await fetch(`${API_URL}/api/v1/favorites/${favoriteId}/user/${parsedUserId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`
        }
  ...
  const handleDeleteNote = async (noteId) => {
    ...
    const parsedUserId = parseInt(userId);
    const response = await fetch(`${API_URL}/api/v1/notes/${noteId}/user/${parsedUserId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`
      }
